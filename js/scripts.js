onload = async (event) =>  {
    ...
    var _countryCode = getCountryCode()
    var countryCode = (_countryCode) ? _countryCode.toString() : "US";
    ...
    if(countryCode in jsonOffers.offers){
        ...
        $("#promoContainer1").addClass('promo-container-disabled');
        $("#promoHeading1").text("No promos")
        $("#promoSubheading1").text("No promos avaiable right now");
    }
    ...
    $(".tik-tok-link").click(function(){
        window.open("https://www.tiktok.com/@davixoffers");
    });

    $(".youtube-link").click(function(){
        window.open("https://www.youtube.com/@davixpromos");
    });
...
function promoClick(){
    if((!selected_promos2) ? indexes1.length < promos1.length : indexes2.length < promos2.length){
        $(".promo2").off('click');
        ...
        while(true){
            if(indexes1.length < promos1.length)
                index = Math.floor(Math.random() * promos1.length);
            else {
                index = Math.floor(Math.random() * promos2.length);
                selected_promos2 = true;
            }
            
            
            if((!selected_promos2) ? !indexes1.includes(index) : !indexes2.includes(index)){
                if(selected_promos2)
                    indexes2.push(index);
                else
                    indexes1.push(index);
                setTimeout(function() {
                    ...
                    setTimeout(function(){
                        ...
                        $(".promo2").click(promoClick);
                        window.open((selected_promos2) ? promos2[index] : promos1[index]);
                    }, 350);
                }, 2000);   
